...
func TestETAWithHistory(t *testing.T) {
	...
	eta, err := stats.CalculateETA(sessions, []int{2, 2}, 6)
	if err != nil {
...
func TestETANoHistory(t *testing.T) {
	_, err := stats.CalculateETA(nil, nil, 5)
	if err == nil {
...
func TestETAZeroRemaining(t *testing.T) {
	...
	eta, err := stats.CalculateETA(sessions, []int{2}, 0)
	if err != nil {
		t.Fatalf("CalculateETA: %v", err)
	}
	if eta != 0.0 {
...
func TestETAFallbackWhenNoTasksCompleted(t *testing.T) {
	...
	eta, err := stats.CalculateETA(sessions, []int{0}, 3)
	if err != nil {
		t.Fatalf("CalculateETA: %v", err)
	}
	if eta != 90.0 {
...
func TestETAAsymmetricHistory(t *testing.T) {
	...
	eta, err := stats.CalculateETA(sessions, []int{1, 3}, 4)
	if err != nil {
		t.Fatalf("CalculateETA: %v", err)
	}
	if eta != 60.0 {
