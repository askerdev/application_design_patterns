...
func TestByStatusFilter(t *testing.T) {
	f := task.ByStatusFilter{Status: domain.TaskStatusTodo}
...
func TestByDateFilter(t *testing.T) {
	...
	to := time.Now()
	f := task.ByDateFilter{From: from, To: to}
	result := f.Filter(makeTasks())
	if len(result) != 1 {
		t.Fatalf("expected 1 task, got %d", len(result))
	}
...
func TestByStatusFilterInProgress(t *testing.T) {
	f := task.ByStatusFilter{Status: domain.TaskStatusInProgress}
	result := f.Filter(makeTasks())
	if len(result) != 1 {
